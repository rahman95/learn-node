...
function handleKeyInputs(el, e) {
  ...
  var _class = 'search__result';
  var current = el.querySelector('.' + _class + '--active');
  var items = el.querySelectorAll('.' + _class);
  var next;
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      if (res.data.length) {
        searchResults.innerHTML = _dompurify2.default.sanitize(resultsToHtml(res.data));
      } else {
  ...
  searchInput.on('keyup', function (e) {
    return handleKeyInputs(search, e);
  });
}
...
var forms = (0, _bling.$$)('form.heart');
forms.on('submit', _heart2.default);
...
function ajaxHeart(e) {
    ...
    var cardHeart = this.heart;

    _axios2.default.post(this.action).then(function (res) {
        var isHearted = cardHeart.classList.toggle('heart__button--hearted');
